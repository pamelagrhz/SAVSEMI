...
namespace SAVSEM
{
	public partial class Buscar : System.Web.UI.Page
	{
		...
		{
			string orderb = String.Empty;
			if (ddlOrder.Text == "Ascendente") {
				orderb = "ASC";
			} else {
				orderb = "DESC";
			}
			ArrayList vendedorID = new ArrayList ();
			cmd.CommandText = "SELECT * FROM Productos WHERE Nombre LIKE '%" + Session["busqueda"].ToString() + "%' ORDER BY " + ddlColumn.Text + " " + orderb + ";";
		...
		protected void txtFilPrec_Click(Object sender, EventArgs e)
		{
			if (txtBusPrec.Text != "") {
				string orderb = String.Empty;
				if (ddlOrderPrec.Text == "Mayor") {
					orderb = ">=";
				} else {
					orderb = "<=";
				}
				ArrayList vendedorID = new ArrayList ();
				cmd.CommandText = "SELECT * FROM Productos WHERE Precio" + orderb + txtBusPrec.Text + ";";
				cmd.Connection = sqlcnx;
				data = cmd.ExecuteReader ();
				if (dst.Tables [0].Rows.Count > 0) {
					dst.Tables [0].Rows.Clear ();
		...
		protected void txtFilCant_Click(Object sender, EventArgs e)
		{
			if (txtBuscCant.Text != "") {
				string orderb = String.Empty;
				if (ddlOrderCant.Text == "Mayor") {
					orderb = ">=";
				} else {
					orderb = "<=";
				}
				ArrayList vendedorID = new ArrayList ();
				cmd.CommandText = "SELECT * FROM Productos WHERE UnidadesDidsponibles" + orderb + txtBuscCant.Text + ";";
				cmd.Connection = sqlcnx;
				data = cmd.ExecuteReader ();
				if (dst.Tables [0].Rows.Count > 0) {
					dst.Tables [0].Rows.Clear ();
		...
		protected void btnFilVend_Click(Object sender, EventArgs e)
		{
			if (txtFilVend.Text != "") {
				string VendedorIDs = string.Empty;
				cmd.CommandText = "SELECT * FROM Usuarios WHERE Nombre LIKE '%" + txtFilVend.Text + "%';";
				cmd.Connection = sqlcnx;
				data = cmd.ExecuteReader ();
				while (data.Read ()) {
					VendedorIDs = data [0].ToString ();
				}
				data.Close ();

				ArrayList vendedorID = new ArrayList ();
				cmd.CommandText = "SELECT * FROM Productos WHERE VendedorID='" + VendedorIDs + "';";
				cmd.Connection = sqlcnx;
				data = cmd.ExecuteReader ();
				if (dst.Tables [0].Rows.Count > 0) {
					dst.Tables [0].Rows.Clear ();
