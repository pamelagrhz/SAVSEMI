...
namespace SAVSEM
{
	public partial class Index : System.Web.UI.MasterPage
	{
		...
		protected void Page_Load(Object sender, EventArgs ev){
			...
			if (Session["user"]!=null) {
				...
				if (Convert.ToBoolean (Session ["admin"].ToString ())) {
					linkAdministrador.Visible = true;
				} else {
					linkAdministrador.Visible = false;
				}
			} else {
				lgUser.Visible = true;
		...
		protected void linkAdministrador_Click(Object sender, EventArgs e)
		{
		...
		private void ChecarExistenciaAdmin (){
			bool admin = false;
			cmd.CommandText = "SELECT * FROM Usuarios WHERE Administrador='True';";
			cmd.Connection = sqlcnx;
			try{
				data = cmd.ExecuteReader();
				while(data.Read()){
					if(Convert.ToBoolean( data[9].ToString())){
						admin = true;
					}
				}
				data.Close();
				if(!admin){
					cmd.CommandText = "INSERT INTO Usuarios(Nombre, Apellidos, Direccion, CP, Telefono, Correo, Usuario, Contrasenia, Administrador, Vendedor) " +
						"VALUES('Default', 'Default', 'Default', 'Default', 'Default', 'Default', 'Admin', '1234', 'True', 'False');";
					cmd.ExecuteNonQuery();
				}
